...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            # next_qs[0, :] = next_qs_min
            # next_qs[1, :] = next_qs_min
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                # print('entropy vs qs', next_action_entropy.shape, next_qs.shape)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # print('act disto:', action_distribution, self.num_actor_samples, self.action_dim)
        actions = action_distribution.rsample([self.num_actor_samples])
        entropy_val = -action_distribution.log_prob(actions)
        # print(entropy_val.shape, actions.shape)
        # print(entropy_val.shape)
        batch_entropy = torch.mean(entropy_val, dim=0)
        # print(batch_entropy.shape)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = torch.stack(
                [
                    torch.stack([critic(obs, act) for act in action], dim=0)
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_values = torch.stack(
            [self.critic(obs, act) for act in action], dim=0
        )
